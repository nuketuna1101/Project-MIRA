...
//    {
//        MIRALOG(Error, TEXT("EnemyCharacterData data not found for EnemyName: %s"), EnemyName);
//        return nullptr;
//    }
//
...
void UMIRAGameInstance::OnDataTablesLoaded()
{
    ...
    MIRACharacterDataTable = Cast<UDataTable>(StaticLoadObject(UDataTable::StaticClass(), nullptr, *DataPath1));
    MIRAEnemyCharacterDataTable = Cast<UDataTable>(StaticLoadObject(UDataTable::StaticClass(), nullptr, *DataPath2));

    if (MIRACharacterDataTable && MIRACharacterDataTable->GetRowMap().Num() > 0 &&
        MIRAEnemyCharacterDataTable && MIRAEnemyCharacterDataTable->GetRowMap().Num() > 0)
    {
        MIRALOG(Warning, TEXT("DT LOAD SUCCESS"));
    }
    else
    {
        MIRALOG(Error, TEXT("FAILED TO LOAD DT"));
    }
}
